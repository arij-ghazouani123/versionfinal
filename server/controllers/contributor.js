...
export async function deleteContributorFromProject   (req, res)  {
 ...
 // console.log(projectId);
  ...
  try {
    ...
    const userValue = Project.user.id;
    ...
    if (!Contributor) {
       await res.status(405).send({ message: "Contributor not found" });
    }
    //console.log(Contributor);

  // Check if the user is a Maintainer in the project

if (userValue !== userId) {
  await    res.status(403).send({ message: "User is not a Maintainer in the project" });
}
...
if (!contributorEmails.includes(Contributor.user.email)) {
  await   res.status(404).send({ message: "Contributor not found in the project" });
}
    ...
    await res.status(200).send({ message: "Contributor deleted from the project" });
  } catch (error) {
    console.log(error);
...
export  async function getContributorRoleByUerID(req, res)  {
  try {
    ...
    const foundContributor = await contributor.findOne({ 'user.id': user }).populate('user');
